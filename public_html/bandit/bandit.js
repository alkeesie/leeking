...
(() => {
  ...
  function mountAd(){
    ...
    if (storedDestRaw) {
      const placeholderPattern = /vi-online-arcade\.example\/?$/i;
      if (placeholderPattern.test(storedDestRaw)) {
        localStorage.removeItem('bandit_dest');
      } else {
        const resolvedDest = resolvePath(storedDestRaw);
        if (resolvedDest && !placeholderPattern.test(resolvedDest)) {
  ...
  window.BanditControls = {
    ...
    setDest(url){
      if (!url) { localStorage.removeItem('bandit_dest'); return; }
      localStorage.setItem('bandit_dest', String(url).trim());
    },
    addArm(arm){
      const list=getArms();
